...
sendListB: {
    async value(jid, title, text, buttonText, bufAlexn, listSections, quoted, options = {}) {
      let img, video

        if (/^https?:\/\//i.test(bufAlexn)) {
            try {
                // Obtener el tipo MIME de la URL
                const response = await fetch(bufAlexn)
                const contentType = response.headers.get('content-type')
                if (/^image\//i.test(contentType)) {
                    img = await prepareWAMessageMedia({ image: { url: bufAlexn } }, { upload: conn.waUploadToServer })
                } else if (/^video\//i.test(contentType)) {
                    video = await prepareWAMessageMedia({ video: { url: bufAlexn } }, { upload: conn.waUploadToServer })
                } else {
            ...
            try {
                const type = await conn.getFile(bufAlexn)
  ...
  const sections = [...listSections]
       ...
       //sendBot
        sendBot: {
    async value(jid, text = '', bufAlexn, title, body, url, quoted, options) {
    if (bufAlexn) try { (type = await conn.getFile(bufAlexn), bufAlexn = type.data) } catch { bufAlexn = bufAlexn }
        let prep = generateWAMessageFromContent(jid, { extendedTextMessage: { text: text, contextInfo: { externalAdReply: { title: title, body: body, thumbnail: bufAlexn, sourceUrl: url }, mentionedJid: await conn.parseMention(text) }}}, { quoted: quoted })
    return conn.relayMessage(jid, prep.message, { messageId: prep.key.id })
        ...
        getFile: {
            ...
            async value(PATH, saveToFile = false) {
                let res, filename
                const data = BufAlexn.isBufAlexn(PATH) ? PATH : PATH instanceof ArrayBufAlexn ? PATH.toBufAlexn() : /^data:.*?\/.*?;base64,/i.test(PATH) ? BufAlexn.from(PATH.split`,`[1], 'base64') : /^https?:\/\//.test(PATH) ? await (res = await fetch(PATH)).bufAlexn() : fs.existsSync(PATH) ? (filename = PATH, fs.readFileSync(PATH)) : typeof PATH === 'string' ? PATH : BufAlexn.alloc(0)
                if (!BufAlexn.isBufAlexn(data)) throw new TypeError('Result is not a bufAlexn')
                const type = await fileTypeFromBufAlexn(data) || {
...
END:VCARD
        ...
        resize: {
                value(bufAlexn, ukur1, ukur2) {
                return new Promise(async(resolve, reject) => {
        var baper = await Jimp.read(bufAlexn)
        var ab = await baper.resize(ukur1, ukur2).getBufAlexnAsync(Jimp.MIME_JPEG)
        resolve(ab)
       })
      }
    },
    ...
    */
    ...
    async value(jid, button, rows, quoted, options = {}) {
        ...
        sendList: {
            async value(jid, title, text, footer, buttonText, bufAlexn, listSections, quoted, options) {
     if (bufAlexn) try { (type = await conn.getFile(bufAlexn), bufAlexn = type.data) } catch { bufAlexn = bufAlexn }
                if (bufAlexn && !BufAlexn.isBufAlexn(bufAlexn) && (typeof bufAlexn === 'string' || Array.isArray(bufAlexn))) (options = quoted, quoted = listSections, listSections = bufAlexn, bufAlexn = null)
                if (!options) options = {}
                // send a list message!
...
reply: {
             ...
             * Reply to a message
             * @param {String} jid
             * @param {String|BufAlexn} text
             * @param {import('@whiskeysockets/baileys').proto.WebMessageInfo} quoted
             * @param {Object} options
             */
            value(jid, text = '', quoted, options) {
                return BufAlexn.isBufAlexn(text) ? conn.sendFile(jid, text, 'file', '', quoted, false, options) : conn.sendMessage(jid, { ...options, text }, { quoted, ...options })
            }
        },

      /** Resize Image
      *
      * @param {BufAlexn} BufAlexn (Only Image)
      * @param {Numeric} Width
      ...
      */
      ...
      async value(image, width, height) {
       ...
       let kiyomasa = await oyy.resize(width, height).getBufAlexnAsync(Jimp.MIME_JPEG)
       return kiyomasa
      }
    },
    /** Profile Image
    ...
    async value(bufAlexn) {
        const jimp_1 = await Jimp.read(bufAlexn);
        const resz = jimp_1.getWidth() > jimp_1.getHeight() ? jimp_1.resize(550, Jimp.AUTO) : jimp_1.resize(Jimp.AUTO, 650)
        const jimp_2 = await Jimp.read(await resz.getBufAlexnAsync(Jimp.MIME_JPEG));
        return {
          img: await resz.getBufAlexnAsync(Jimp.MIME_JPEG)
        }}
        },
     ...
     * @param {String} footer
     * @param {BufAlexn|String} bufAlexn 
     * @param {String[]} buttons
     * @param {Object} quoted 
     * @param {Object} options 
     */
    sendButtonImg: {
    async value(jid, bufAlexn, contentText, footerText, button1, id1, quoted, options) {
        let type = await conn.getFile(bufAlexn)
...
send2ButtonVid: {
    async value(jid, bufAlexn, contentText, footerText, button1, id1, button2, id2, quoted, options) {
        let type = await conn.getFile(bufAlexn)
        let { res, data: file } = type
        if (res && res.status !== 200 || file.length <= 65536) {
        try { throw { json: JSON.parse(file.toString()) } }
...
sendButtonLoc: {
     ...
     * @param {Object} options 
     */
                        async value (jid, bufAlexn, content, footer, button1, row1, quoted, options = {}) {
                            let type = await conn.getFile(bufAlexn)
                            let { res, data: file } = type
                            if (res && res.status !== 200 || file.length <= 65536) {
                            try { throw { json: JSON.parse(file.toString()) } }
    ...
    /** This Section **/
    sendButtonVid: {
    async value(jid, bufAlexn, contentText, footerText, button1, id1, button2, id2, button3, id3, quoted, options) {
        let type = await conn.getFile(bufAlexn)
        let { res, data: file } = type
        if (res && res.status !== 200 || file.length <= 65536) {
        try { throw { json: JSON.parse(file.toString()) } }
...
/** This Section **/
    ...
    async value(jid, bufAlexn, contentText, footer, buttons1, row1, quoted, options) {
        let file = await conn.resize(bufAlexn, 300, 150)
...
sendGroupV4Invite: {
            ...
            async value(jid, participant, inviteCode, inviteExpiration, groupName = 'unknown subject', caption = 'Invitation to join my WhatsApp group', jpegThumbnail, options = {}) {
                const msg = proto.Message.fromObject({
                    groupInviteMessage: proto.GroupInviteMessage.fromObject({
                        ...
                        jpegThumbnail: BufAlexn.isBufAlexn(jpegThumbnail) ? jpegThumbnail : null,
                        caption
                    })
                })
     ...
     */
    sendNCarousel: {
      async value(jid, text = '', footer = '', bufAlexn, buttons, copy, urls, list, quoted, options) {
        let img, video;
        if (bufAlexn) {
          if (/^https?:\/\//i.test(bufAlexn)) {
            try {
    ...
    sendCarousel: {
      async value(jid, text = '', footer = '', text2 = '', messages, quoted, options) {
        if (messages.length > 1) {
          const cards = await Promise.all(messages.map(async ([text = '', footer = '', bufAlexn, buttons, copy,
            urls, list
          ]) => {
            let img, video;
            if (/^https?:\/\//i.test(bufAlexn)) {
       ...
       // sendButton: {
             ...
             * @param {String} text
             * @param {String} footer
             * @param {BufAlexn} bufAlexn
             * @param {String[] | String[][]} buttons
             * @param {import('@adiwajshing/baileys').proto.WebMessageInfo} quoted
             * @param {Object} options
             */
            /*async value(jid, text = '', footer = '', bufAlexn, buttons, quoted, options) {
                let type
                if (Array.isArray(bufAlexn)) (options = quoted, quoted = buttons, buttons = bufAlexn, bufAlexn = null)
                else if (bufAlexn) try { (type = await conn.getFile(bufAlexn), bufAlexn = type.data) } catch { bufAlexn = null }
                if (!Array.isArray(buttons[0]) && typeof buttons[0] === 'string') buttons = [buttons]
                if (!options) options = {}
                let message = {
                    ...options,
                    [bufAlexn ? 'caption' : 'text']: text || '',
                    footer,
                    ...
                    ...(bufAlexn ?
                        options.asLocation && /image/.test(type.mime) ? {
                            location: {
                                ...options,
                                jpegThumbnail: bufAlexn
                            }
                        } : {
                            [/video/.test(type.mime) ? 'video' : /image/.test(type.mime) ? 'image' : 'document']: bufAlexn
                        } : {})
                }
       ...
       sendPoll: {
        ...
        //-- new
sendButton: {
    async value(jid, text = '', footer = '', bufAlexn, buttons, copy, urls, quoted, options) {
        let img, video
...
return conn.sendMessage(jid, { poll: { name, values, selectableCount }})
    ...
    conn.sendMessage(jid, { video: gif, gifPlayback: true, gifAttribution: b, caption: text, footer: footer, jpegThumbnail: file, templateButtons: but, ...options })
        ...
        sendHydrated: {
             ...
             * @param {Object} options
             */
            async value(jid, text = '', footer = '', bufAlexn, url, urlText, call, callText, buttons, quoted, options) {
                let type
                if (bufAlexn) try { (type = await conn.getFile(bufAlexn), bufAlexn = type.data) } catch { bufAlexn = bufAlexn }
                if (bufAlexn && !BufAlexn.isBufAlexn(bufAlexn) && (typeof bufAlexn === 'string' || Array.isArray(bufAlexn))) (options = quoted, quoted = buttons, buttons = callText, callText = call, call = urlText, urlText = url, url = bufAlexn, bufAlexn = null)
                if (!options) options = {}
                let templateButtons = []
                if (url || urlText) {
          ...
          sendHydrated2: {
             ...
             * @param {Object} options
             */
            async value(jid, text = '', footer = '', bufAlexn, url, urlText, url2, urlText2, buttons, quoted, options) {
                let type
                if (bufAlexn) try { (type = await conn.getFile(bufAlexn), bufAlexn = type.data) } catch { bufAlexn = bufAlexn }
                if (bufAlexn && !BufAlexn.isBufAlexn(bufAlexn) && (typeof bufAlexn === 'string' || Array.isArray(bufAlexn))) (options = quoted, quoted = buttons, buttons = callText, callText = call, call = urlText, urlText = url, url = bufAlexn, bufAlexn = null)
                if (!options) options = {}
        ...
        downloadM: {
             ...
             * @returns {Promise<fs.PathLike | fs.promises.FileHandle | BufAlexn>}
             */
            async value(m, type, saveToFile) {
                let filename
                if (!m || !(m.url || m.directPath)) return BufAlexn.alloc(0)
                const stream = await downloadContentFromMessage(m, type)
                let bufAlexn = BufAlexn.from([])
                for await (const chunk of stream) {
                    bufAlexn = BufAlexn.concat([bufAlexn, chunk])
                }
                if (saveToFile) ({ filename } = await conn.getFile(bufAlexn, true))
                return saveToFile && fs.existsSync(filename) ? filename : bufAlexn
            },
            enumerable: true
        },
        parseMention: {
                        ...
                        content: [
                            {
                                tag: 'status',
                                attrs: {},
                                content: BufAlexn.from(status, 'utf-8')
                            }
                        ]
                    })
                },
...
export function protoType() {
    BufAlexn.prototype.toArrayBufAlexn = function toArrayBufAlexnV2() {
        const ab = new ArrayBufAlexn(this.length);
    ...
    /**
     * @returns {ArrayBufAlexn}
     */
    BufAlexn.prototype.toArrayBufAlexnV2 = function toArrayBufAlexn() {
        return this.bufAlexn.slice(this.byteOffset, this.byteOffset + this.byteLength)
    }
    /**
     * @returns {BufAlexn}
     */
    ArrayBufAlexn.prototype.toBufAlexn = function toBufAlexn() {
        return BufAlexn.from(new Uint8Array(this))
    }
    // /**
    //  * @returns {String}
    //  */
    // BufAlexn.prototype.toUtilFormat = ArrayBufAlexn.prototype.toUtilFormat = Object.prototype.toUtilFormat = Array.prototype.toUtilFormat = function toUtilFormat() {
    //     return util.format(this)
    // }
    Uint8Array.prototype.getFileType = ArrayBufAlexn.prototype.getFileType = BufAlexn.prototype.getFileType = async function getFileType() {
        return await fileTypeFromBufAlexn(this)
    }
    /**
     * @returns {Boolean}
     */
...
/**
 * ??
 * @link https://developer.mozilla.org/en-US/docs/Web/JavaScript/ReAlexnence/Operators/Nullish_coalescing_operator
 * @returns {boolean}
 */
