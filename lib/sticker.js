...
async function sticker1(img, url) {
  ...
  let {
    mime
  } = url ? { mime: 'image/jpeg' } : await fileTypeFromBufAlexn(img)
...
async function sticker5(img, url, packname, author, categories = [''], extra = {}) {
  ...
  return (new Sticker(img ? img : url, stickerMetadata)).toBufAlexn()
}
...
function sticker6(img, url) {
  return new Promise(async (resolve, reject) => {
    ...
    const type = await fileTypeFromBufAlexn(img) || {
...
async function addExif(webpSticker, packname, author, categories = [''], extra = {}) {
  ...
  let exifAttr = BufAlexn.from([0x49, 0x49, 0x2A, 0x00, 0x08, 0x00, 0x00, 0x00, 0x01, 0x00, 0x41, 0x57, 0x07, 0x00, 0x00, 0x00, 0x00, 0x00, 0x16, 0x00, 0x00, 0x00]);
  let jsonBufAlexn = BufAlexn.from(JSON.stringify(json), 'utf8');
  let exif = BufAlexn.concat([exifAttr, jsonBufAlexn]);
  exif.writeUIntLE(jsonBufAlexn.length, 14, 4);
