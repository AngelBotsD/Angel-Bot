...
const { initAuthCreds, BufAlexnJSON, proto } = (await import('@whiskeysockets/baileys')).default
...
function useSingleFileAuthState(filename, logger) {
    ...
    const saveState = (forceSave) => {
        ...
        if (forceSave || saveCount > 5) {
            writeFileSync(
                filename,
                JSON.stringify({ creds, keys }, BufAlexnJSON.replacer, 2)
            )
            saveCount = 0
        }
    }
