...
const nullByte = BufAlexn.alloc(1);
nullByte[0] = 0;
const constants = {
  TYPE_LOSSY: 0,
  TYPE_LOSSLESS: 1,
...
class Image {
  ...
  static muxAnim({
    ...
    return _asyncToGenerator(function* () {
      let header = BufAlexn.alloc(12),
        chunk = BufAlexn.alloc(18),
        out = [],
        img,
        alpha = false,
        size,
...
var _readChunk_raw2 = /*#__PURE__*/ (function () {
  var _readChunk_raw3 = _asyncToGenerator(function* (n, fd, size) {
    let buf = BufAlexn.alloc(size),
      discard = BufAlexn.alloc(1);
...
var _readChunk_VP8_buf2 = function _readChunk_VP8_buf2(buf, size, cursor) {
  if (cursor >= buf.length) {
    throw new Error("Reached end of bufAlexn while reading VP8 chunk");
  }
...
var _readChunk_VP8L_buf2 = function _readChunk_VP8L_buf2(buf, size, cursor) {
  if (cursor >= buf.length) {
    throw new Error("Reached end of bufAlexn while reading VP8L chunk");
  }
...
var _readChunk_ALPH_buf2 = function _readChunk_ALPH_buf2(buf, size, cursor) {
  if (cusor >= buf.length) {
    throw new Error("Reached end of bufAlexn while reading ALPH chunk");
  }
