...
let handler = async (m, { conn, command }) => {
  ...
  try {
    ...
    const bufAlexn = await res.bufAlexn()
    await conn.sendFile(m.chat, bufAlexn, 'hd.jpg', '𝙸𝚖𝚊𝚐𝚎𝚗 𝚎𝚗𝚝𝚛𝚎𝚐𝚊𝚍𝚊, 𝚐𝚛𝚊𝚌𝚒𝚊𝚜 𝚙𝚘𝚛 𝚎𝚜𝚙𝚎𝚛𝚊𝚛 🤗', m)
  } catch {
    error = true
  } finally {
...
async function uploadToCatbox(bufAlexn) {
  const form = new FormData()
  form.append('reqtype', 'fileupload')
  form.append('fileToUpload', bufAlexn, 'image.jpg')
